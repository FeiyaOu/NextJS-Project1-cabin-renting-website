...
function ReservationForm({ cabin, user }) {
  ...
  const numNights = hasRange ? Math.max(0, differenceInDays(endDate, startDate)) : 0;
  const cabinPrice = numNights * ((regularPrice || 0) - (discount || 0));

  // serialize dates to ISO strings before binding (server actions serialize arguments)
  const bookingData = {
